        ...
        protected class SqlText
        {
            public SqlText(string repositoryName)
            {
                ...
                CreateJournalTableCommand = FormatCommand(
                    @"IF  NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'Journal{0}') AND type in (N'U'))
	CREATE TABLE [Journal{0}](
        ...
        [Timestamp] [timestamp] NOT NULL
                ...
                CreateSnapshotTableCommand = FormatCommand(
                    @"IF  NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'Snapshot{0}') AND type in (N'U'))
	CREATE TABLE [Snapshot{0}](
		[Revision] [int] NOT NULL PRIMARY KEY,
		[ModelJson] [ntext] NOT NULL,
        [Timestamp] [timestamp] NOT NULL
	)");
                LogCommand = FormatCommand(@"INSERT INTO [Journal{0}] (Type, CommandJson) VALUES (@Type, @CommandJson)");
                SaveSnapshot =
                    FormatCommand(
                        @"INSERT INTO [Snapshot{0}] (Revision, ModelJson) SELECT TOP 1 Revision,@ModelJson FROM [Journal{0}] ORDER BY Revision DESC");

                LastSnapshot = FormatCommand(@"SELECT Revision, ModelJson FROM [Snapshot{0}] ORDER BY Revision DESC");
            ...
            {
                return string.Format(sql, (string.IsNullOrEmpty(RepositoryName) ? "" : "_" + RepositoryName));
            }
        }
